...
#endif
        std::priority_queue<std::pair<float, labeltype >> result = appr_alg.searchKnnRes(massQ + vecdim * i,
                                                                                         RandQ + B * i, k);
#ifndef WIN32
        GetCurTime(&run_end);
...
int main(int argc, char *argv[]) {
    ...
    while (iarg != -1) {
        iarg = getopt_long(argc, argv, "d:b:k:s:r:", longopts, &ind);
        switch (iarg) {
            case 'k':
    ...
    sprintf(result_file_view, "%s%s_hnsw_res.log", result_path, dataset);
    ...
    if (str_data == "msong") {
        ...
        test_vs_recall(PCAQ.data, RandQ.data, appr_alg->max_elements_, Q.n, *appr_alg, Q.d, answers, subk);
    }
    if (str_data == "gist") {
