...
public:
    ...
    GraphRes(const Matrix<float> &X);

    GraphRes(const Matrix<float> &X, const Matrix<float> &_centroids,
    ...
    void SearchRes(float *query, float *rd_query, int k, int L, ResultHeap &KNNs) const;
...
template<uint32_t D, uint32_t B>
GraphRes<D, B>::GraphRes(const Matrix<float> &X) {
    nd_ = X.n;
...
template<uint32_t D, uint32_t B>
GraphRes<D, B>::GraphRes(const Matrix<float> &X, const Matrix<float> &centroid,
    ...
    for (int i = 0; i < nd_; i++) {
        ...
        fac[i].sqr_x = x2.data[i] * x2.data[i] +
                       ip_sim<D - B>(data_ + i * D + B, data_ + i * D + B); // add the residual dim norm
...
template<uint32_t D, uint32_t B>
void GraphRes<D, B>::LoadGraph(const char *filename) {
...
template<uint32_t D, uint32_t B>
void GraphRes<D, B>::Search(float *query, int K, int L, ResultHeap &KNNs) const {
...
template<uint32_t D, uint32_t B>
void GraphRes<D, B>::SearchRes(float *query, float *rd_query, int K, int L, ResultHeap &KNNs) const {
    std::vector<Neighbor> retset(L + 1);
    std::vector<unsigned> init_ids(L);
    boost::dynamic_bitset<> flags{nd_, 0};
    std::mt19937 rng(rand());
    ...
    res_error = 3.0f * std::sqrt(res_error);
    float sqr_y = sqr_dist<B>(rd_query, centroid_);
    float y = std::sqrt(sqr_y);
    sqr_y += ip_sim<D - B>(query + B, query + B);
    ...
    while (k < (int) L) {
        ...
        if (retset[k].flag) {
            ...
            for (unsigned m = 0; m < final_graph_[n].size(); ++m) {
                ...
                float error_bound = y * (fac[id].error) + res_error;
...
template<uint32_t D, uint32_t B>
void GraphRes<D, B>::uniform_prune_parameter_config(const Matrix<float> &X) {
    /* Configure Prune Parameters */
    std::vector<long double> mean(D);
#pragma omp parallel for schedule(dynamic, 70)
    ...
    mean_ = new float[D];
    for (int j = 0; j < D; j++) mean_[j] = mean[j] / nd_;
#pragma omp parallel for schedule(dynamic, 70)
    for (int j = 0; j < D; j++) {
        for (int i = 0; i < nd_; i++) {
...
#pragma omp parallel for schedule(dynamic, 70)
    ...
    var_ = new float[D];
    for (int j = 0; j < D; j++) var_[j] = variance[j] / nd_;
    std::cerr << "Parameter Configure Finished" << std::endl;
}

template<uint32_t D, uint32_t B>
void GraphRes<D, B>::Load(const char *filename) {
...
template<uint32_t D, uint32_t B>
void GraphRes<D, B>::Save(const char *filename) {
