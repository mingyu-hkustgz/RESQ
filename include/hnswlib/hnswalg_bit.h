//
// Created by Mingyu on 24-11-11.
//

#ifndef RESQ_HNSWALG_BIT_H
struct Factor {
    float sqr_x;
    float error;
    float factor_ppc;
...
namespace hnswlib {
    ...
    template<typename dist_t, uint32_t D, uint32_t B>
    class HierarchicalNSWBitQ : public AlgorithmInterface<dist_t> {
    public:
        ...
        int residual_minimized_flag = 0;
        std::mutex deleted_elements_lock;  // lock for deleted_elements
        std::unordered_set<tableint> deleted_elements;  // contains internal ids of deleted elements

        /* Begin quantized code file*/
        ...
        /* End quantized code file*/

        HierarchicalNSWBitQ(SpaceInterface<dist_t> *s) {
        }
        ...
        void rabit_quantized_init(const Matrix<float> &centroid,
                                  const Matrix<float> &x2, const Matrix<float> &_x0,
                                  const Matrix <uint64_t> &binary) {
            x0 = new float[cur_element_count];
            centroid_ = new float[B];
            binary_code = new uint64_t[cur_element_count * B / 64];
            fac = new Factor[cur_element_count];
            std::memcpy(centroid_, centroid.data, B * sizeof(float));

            for (size_t i = 0; i < cur_element_count; i++) {
                size_t ex_id = getExternalLabel(i);
                std::memcpy(x0 + i, _x0.data + ex_id, sizeof(float));
                std::memcpy(binary_code + i * (B / 64), binary.data + ex_id * (B / 64), (B / 64) * sizeof(uint64_t));
            }
            u = new float[B];
    ...
    for(int i=0;i<B;i++)u[i] = uniform(gen);
            ...
            for (int i = 0; i < cur_element_count; i++) {
                size_t ex_id = getExternalLabel(i);
                long double x_x0 = (long double) x2.data[ex_id] / x0[i];
                fac[i].sqr_x = x2.data[ex_id] * x2.data[ex_id];
                fac[i].error = 2 * max_x1 * std::sqrt(x_x0 * x_x0 - x2.data[ex_id] * x2.data[ex_id]);
            ...
            ave_error /= cur_element_count;
            std::cerr << "ave error:: " << ave_error<< std::endl;
        }

        void uniform_prune_parameter_config(const Matrix<float> &M) {
        ...
        void res_quantized_init(const Matrix<float> &centroid, const Matrix<float> &x2, const Matrix<float> &_x0,
                                const Matrix <uint64_t> &binary, const Matrix<float> M) {
            x0 = new float[cur_element_count];
            centroid_ = new float[B];
            binary_code = new uint64_t[cur_element_count * B / 64];
            fac = new Factor[cur_element_count];
    ...
    for(int i=0;i<B;i++)u[i] = uniform(gen);
            ...
            for (int i = 0; i < cur_element_count; i++) {
                ...
                long double x_x0 = (long double) x2.data[ex_id] / x0[i];
                fac[i].sqr_x = x2.data[ex_id] * x2.data[ex_id] +
                               ip_sim<D - B>(((float *) getDataByInternalId(i)) + B,
                                             ((float *) getDataByInternalId(i)) + B); // add the residual dim norm
                fac[i].error = 2 * max_x1 * std::sqrt(x_x0 * x_x0 - x2.data[ex_id] * x2.data[ex_id]);
                fac[i].factor_ppc = -2 / fac_norm * x_x0 * ((float) space.popcount(binary_code + i * B / 64) * 2 - B);
                fac[i].factor_ip = -2 / fac_norm * x_x0;
                ave_error += fac[i].error;
            ...
            ave_error /= cur_element_count;
            residual_minimized_flag = 1; // mark use MRQ
            std::cerr << "ave error:: " << ave_error<< std::endl;
        }
...
//                    if (candidate_id == 0) continue;
        ...
        searchBaseLayerRabit(
                tableint ep_id,
                const void *data_point,
                size_t ef, size_t k, uint64_t *quant_query,
                const float sqr_y, const float y, const float vl, const float width, const float sumq,
                BaseFilterFunctor *isIdAllowed = nullptr,
                BaseSearchStopCondition<dist_t> *stop_condition = nullptr) const {
            VisitedList *vlist = visited_list_pool_->getFreeVisitedList();
            vl_type *visited_array = vlist->mass;
            vl_type visited_array_tag = vlist->curV;
                ...
                for (size_t j = 1; j <= size; j++) {
                    ...
                    if (!(visited_array[candidate_id] == visited_array_tag)) {
                        ...
                        if (answers.size() < k) {
                            ...
                            float temp_dist = (fac[candidate_id].sqr_x) + sqr_y + fac[candidate_id].factor_ppc * vl +
                                              (space.ip_byte_bin(quant_query, binary_code + candidate_id * (B / 64)) *
                                               2 - sumq) *
                                              (fac[candidate_id].factor_ip) * width;
                            float error_bound = y * (fac[candidate_id].error);

                            // If it's a positive object, then include it in R1, R2, S.
                            if (temp_dist - error_bound < lowerBound) {
                                char *currObj1 = (getDataByInternalId(candidate_id));
                                dist_t dist = fstdistfunc_(data_point, currObj1, dist_func_param_);
                                candidate_set.emplace(-dist, candidate_id);
                                if (!isMarkedDeleted(candidate_id)) {
                                ...
                                if (top_candidates.size() < ef || lowerBoundcan > temp_dist) {
                                    top_candidates.emplace(temp_dist, candidate_id);
                                    candidate_set.emplace(-temp_dist, candidate_id);
                                }
                                if (top_candidates.size() > ef) {
                                    top_candidates.pop();
                                }
            ...
            visited_list_pool_->releaseVisitedList(vlist);
            return answers;
        }
                        ...
                        if (answers.size() < k) {
                            ...
                            float temp_dist = (fac[candidate_id].sqr_x) + sqr_y + fac[candidate_id].factor_ppc * vl +
                                              (space.ip_byte_bin(quant_query, binary_code + candidate_id * (B / 64)) *
                                               2 - sumq) *
                                              (fac[candidate_id].factor_ip) * width;
                            float error_bound = y * (fac[candidate_id].error) + res_error;
        ...
        void saveIndex(const std::string &location) {
            ...
            output.write((char *) x0, cur_element_count * sizeof(float));
            output.write((char *) centroid_, B * sizeof(float));
            output.write((char *) binary_code, cur_element_count * B / 64 * sizeof(uint64_t));
            output.write((char *) &residual_minimized_flag, sizeof(int));
            if (residual_minimized_flag) {
...
//                throw std::runtime_error("Index seems to be corrupted or unsupported");
    ...
    for(int i=0;i<B;i++)u[i] = uniform(gen);
            ...
            centroid_ = new float[B];
            binary_code = static_cast<uint64_t *>(aligned_alloc(256, cur_element_count * B / 64 * sizeof(uint64_t)));
            x0 = new float[cur_element_count];
            fac = new Factor[cur_element_count];

            input.read((char *) x0, cur_element_count * sizeof(float));
            input.read((char *) centroid_, B * sizeof(float));
            input.read((char *) binary_code, cur_element_count * B / 64 * sizeof(uint64_t));
            input.read((char *) &residual_minimized_flag, sizeof(int));
            if (residual_minimized_flag) {
                mean_ = new float[D];
                var_ = new float[D];
            ...
            std::cerr << "ave error: " << ave_error / cur_element_count << std::endl;
        ...
        */
        ...
        std::priority_queue<std::pair<dist_t, labeltype >>
        searchKnnRaBit(float *query_data, float *rd_query, size_t k,
            ...
            for (int level = maxlevel_; level > 0; level--) {
                ...
                while (changed) {
                    ...
                    for (int i = 0; i < size; i++) {
                        ...
                        if (cand < 0 || cand > max_elements_)
                            throw std::runtime_error("cand error");
                        float tmp_dist = (fac[cand].sqr_x) + sqr_y + fac[cand].factor_ppc * vl +
                                         (space.ip_byte_bin(quant_query, binary_code + cand * (B / 64)) * 2 - sumq) *
                                         (fac[cand].factor_ip) * width;
                        float error_bound = y * (fac[cand].error);
                        if (tmp_dist - error_bound < curdist) {
                            dist_t d = fstdistfunc_(query_data, getDataByInternalId(cand), dist_func_param_);
                            if (d < curdist) {
                                curdist = d;
                                currObj = cand;
            ...
            bool bare_bone_search = !num_deleted_ && !isIdAllowed;
            if (bare_bone_search) {
                top_candidates = searchBaseLayerRabit<true>(
                        currObj, query_data, std::max(ef_, k), k, quant_query, sqr_y, y, vl, width, sum_q, isIdAllowed);
            } else {
                top_candidates = searchBaseLayerRabit<false>(
                        currObj, query_data, std::max(ef_, k), k, quant_query, sqr_y, y, vl, width, sum_q, isIdAllowed);
            }
            while (top_candidates.size() > k) {
                top_candidates.pop();
        ...
        std::priority_queue<std::pair<dist_t, labeltype >>
        searchKnnRes(float *query_data, float *rd_query, size_t k,
                     BaseFilterFunctor *isIdAllowed = nullptr) const {
            uint8_t  PORTABLE_ALIGN64 byte_query[B];
            // =======================================================================================================
            // Preprocess the residual query and the quantized query
            ...
            float sqr_y = sqr_dist<B>(rd_query, centroid_);
            float y = std::sqrt(sqr_y);
            sqr_y += ip_sim<D - B>(query_data + B, query_data + B);
            // compute residual error
            float res_error = 0;
            for (int i = B; i < D; i++) {
                res_error += var_[i] * query_data[i] * query_data[i];
            }
                ...
                while (changed) {
                    ...
                    for (int i = 0; i < size; i++) {
                        ...
                        float tmp_dist = (fac[cand].sqr_x) + sqr_y + fac[cand].factor_ppc * vl +
                                         (space.ip_byte_bin(quant_query, binary_code + cand * (B / 64)) * 2 - sumq) *
                                         (fac[cand].factor_ip) * width;
                        float error_bound = y * (fac[cand].error) + res_error;
                        if (tmp_dist - error_bound < curdist) {
                            dist_t d = fstdistfunc_(query_data, getDataByInternalId(cand), dist_func_param_);
                            if (d < curdist) {
                                curdist = d;
            ...
            bool bare_bone_search = !num_deleted_ && !isIdAllowed;
            if (bare_bone_search) {
                top_candidates = searchBaseLayerRes<true>(
                        currObj, query_data, std::max(ef_, k), k, quant_query, sqr_y, y, vl, width, sumq, res_error,
                        isIdAllowed);
            } else {
                top_candidates = searchBaseLayerRes<false>(
                        currObj, query_data, std::max(ef_, k), k, quant_query, sqr_y, y, vl, width, sumq, res_error,
                        isIdAllowed);
            }
